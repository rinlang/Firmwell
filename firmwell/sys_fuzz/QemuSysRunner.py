...
class QemuSysRunner(RehostingEnv):
    ...
    def __init__(self, network_config, base_path, fs_path, bin_path, qemu_arch, name, debug, hash, brand,
        ...
        print(f"firmae_path: {self.firmae_path}")
        
        try:
            print(os.listdir(self.firmae_path))
        except Exception as e:
            print(f"Error listing firmae_path: {e}")
    ...
    def _init_sys_fuzz(self):
        """Initialize SYS_FUZZ feature - copy qemu files to fs_path"""
        global SYS_FUZZ
        if not SYS_FUZZ:
            return
            
        print("[SYS_FUZZ] Initializing system fuzzing mode...")
        
        # Map architecture names for source directory
        arch_map = {
            "mipseb": "mips",
            "mipsel": "mipsel", 
            "armel": "arm"
        ...
        if self.arch not in arch_map:
            print(f"[SYS_FUZZ] Unsupported architecture: {self.arch}")
            return
            
            
        source_arch = arch_map[self.arch]
        source_dir = f"/qemu_system_files/fuzz/{source_arch}"
        
        if not os.path.exists(source_dir):
            print(f"[SYS_FUZZ] Source directory {source_dir} does not exist")
            return
            
        print(f"[SYS_FUZZ] Copying files from {source_dir} to {self.fs_path}")
        
        try:
            # Copy all files from source directory to fs_path
            for item in os.listdir(source_dir):
                src_path = os.path.join(source_dir, item)
                dst_path = os.path.join(self.fs_path, item)
                
                if os.path.isfile(src_path):
                    shutil.copy2(src_path, dst_path)
                    # Make executables executable
                    if item.startswith("qemu-system") or item.startswith("vmlinux"):
                        os.chmod(dst_path, 0o755)
                    print(f"[SYS_FUZZ] Copied {item}")
                elif os.path.isdir(src_path):
                    if os.path.exists(dst_path):
                        shutil.rmtree(dst_path)
                    shutil.copytree(src_path, dst_path)
                    print(f"[SYS_FUZZ] Copied directory {item}")
                    
        except Exception as e:
            print(f"[SYS_FUZZ] Error copying files: {e}")
            return
        
        source_dir2 = f"/qemu_system_files/fuzz"
        ...
        try:
            ...
            for item in os.listdir(source_dir2):
                src_path = os.path.join(source_dir2, item)
                dst_path = os.path.join(self.fs_path, item)
                ...
                if os.path.isfile(src_path):
                    ...
                    if item.startswith("qemu-system") or item.startswith("vmlinux"):
                        os.chmod(dst_path, 0o755)
                    print(f"[SYS_FUZZ] Copied {item}")
                elif os.path.isdir(src_path) and item not in ["mips", "mipsel", "arm"]:
                    if os.path.exists(dst_path):
                        shutil.rmtree(dst_path)
                    shutil.copytree(src_path, dst_path)
                    print(f"[SYS_FUZZ] Copied directory {item}")
        ...
        print("[SYS_FUZZ] File copying completed")
        
        # Create FirmAFL configuration and directories
        self._create_firmafl_config()
        self._create_fuzzing_directories()
    
    def _create_firmafl_config(self):
        """Create FirmAFL_config file in fs_path"""
        global SYS_FUZZ
        if not SYS_FUZZ:
            return
            
        print("[SYS_FUZZ] Creating FirmAFL_config file...")
        
        # Get relative directory name for the image path
        # For example, if jobindex is "10853", create "../image_10853"
        # relative_dirname = f"../image_{self.jobindex}"
        relative_dirname = "../fs"
        
        # FirmAFL configuration content
        firmafl_config_content = f"""mapping_filename={relative_dirname}/mapping_table
init_read_pipename={relative_dirname}/user_cpu_state
write_pipename={relative_dirname}/full_cpu_state
program_analysis={os.path.basename(self.httpd_path)}
feed_type=FEED_HTTP
id={self.jobindex}"""
# opti=yes"""
        
        try:
            config_file_path = os.path.join(self.fs_path, "FirmAFL_config")
            with open(config_file_path, 'w') as f:
                f.write(firmafl_config_content)
            print(f"[SYS_FUZZ] Created FirmAFL_config at {config_file_path}")
        except Exception as e:
            print(f"[SYS_FUZZ] Error creating FirmAFL_config: {e}")
    
    def _create_fuzzing_directories(self):
        """Create fuzzing directories and files in fs_path"""
        global SYS_FUZZ
        if not SYS_FUZZ:
            return
            
        print("[SYS_FUZZ] Creating fuzzing directories and files...")
        
        try:
            # Create outputs_full directory
            outputs_full_dir = os.path.join(self.fs_path, "outputs_full")
            os.makedirs(outputs_full_dir, exist_ok=True)

            print(f"[SYS_FUZZ] Created directories: {outputs_full_dir}")
            
        except Exception as e:
            print(f"[SYS_FUZZ] Error creating fuzzing directories: {e}")
    ...
    def start_rehosting_env(self, dest=None, ports=None, potential_urls=None, mac=None,
        ...
        while restart_count <= max_restart_attempts:
            ...
            if self._start_qemu_instance(target_ip, expected_subnet, restart_count):
                ...
                # if self.system_flag:
                time.sleep(10)
                self.exec(f"/firmadyne/busybox wget 127.0.0.1", detach=True)
    ...
    def _start_qemu_instance(self, target_ip: str, expected_subnet: str, restart_count: int) -> bool:
        ...
        # if self.arch == "armel":
        self.log_file = open("/tmp/qemu.final.serial.log", "a")
        
        global SYS_FUZZ
        if SYS_FUZZ:
            # Execute command in fs_path directory for fuzzing mode
            print(f"[SYS_FUZZ] Executing QEMU command in directory: {self.fs_path}")
            self.session = subprocess.Popen(self.qemu_cmd, shell=True, stdout=self.log_file, stderr=self.log_file, cwd=self.fs_path)
        else:
            # Execute command in current directory for normal mode
        ...
        # Flush the log file before reading
        if self.log_file:
            self.log_file.flush()
        ...
        with open("/tmp/qemu.final.serial.log", "rb") as f:
            ...
            decoded_lines = [line.decode('utf-8', errors='ignore') for line in lines[-50:]]
    ...
    def _setup_mipseb_command(self, image: str):
        ...
        if SYS_FUZZ:
            # Use files from fs_path for fuzzing mode
            qemu_binary = "./qemu-system-mips"
            kernel = "./vmlinux.mips_3.2.1"  # Adjust kernel name as needed
            console_setting = "console=ttyS0,115200 console=ttyS1,115200"
        else:
            # Use original paths
            qemu_binary = self.qemu_arch
            kernel = os.path.join(self.binary_path, "vmlinux.mipseb.4")
            console_setting = "console=ttyS0"
        ...
        base_qemu_cmd = (
            ...
            f'-append "root=/dev/sda1 {console_setting} nandsim.parts=64,64,64,64,64,64,64,64,64,64 '
        ...
        if SYS_FUZZ:
            # Remove file serial for fuzzing mode and modify serial/monitor paths
            base_qemu_cmd += (
                ...
                f'-display none -aflFile @@'
            )
            
            if os.path.exists(os.path.join(self.fs_path, "outputs_full")):
                shutil.rmtree(os.path.join(self.fs_path, "outputs_full"))
            os.makedirs(os.path.join(self.fs_path, "outputs_full"), exist_ok=True)
            
            # Wrap with afl-fuzz-full
            self.qemu_cmd = (
                f'./afl-fuzz-full -m none -t 800000+ -i ./inputs -o ./outputs_full -x keywords -QQ -- '
                f'{base_qemu_cmd}'
            )
        else:
            base_qemu_cmd += (
    ...
    def _setup_mipsel_command(self, image: str):
        ...
        if SYS_FUZZ:
            ...
            kernel = "./vmlinux.mipsel_3.2.1"  # Adjust kernel name as needed
            console_setting = "console=ttyS0,115200 console=ttyS1,115200"
            rdinit = "/firmadyne/sh"
        else:
            # Use original paths
    ...
    def _configure_network(self, target_ip: str, expected_subnet: str, restart_count: int) -> bool:
        ...
        self.exec("ifconfig lo 127.0.0.1 up")
